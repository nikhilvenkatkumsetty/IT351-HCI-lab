var level3State = {
	create: function(){
		...
		this.levelLabel = game.add.text(30,10,'Hit Green Balloons! Level 3', {font : '18px Arial', fill: '#000'});
		this.scoreLabel = game.add.text(30,30,'Green Balloons Exploded: 0', {font : '18px Arial', fill: '#000'});
		this.predictedTPLabel = game.add.text(30,50,'Predicted Throughput: ' + (Math.log2(16)/4).toFixed(2), {font : '18px Arial', fill: '#000'});
		...
		for(var i=0; i<balloonAmount; i++){
			...
			else 
			{
				...
				if(i % 3 ==  1) {
					...
					balloon = this.balloonGroup.create(x_others, 480, "balloonyellow");
				} else {
	...
	showScoreWin: function(){
		...
		this.lifeFinalScore = game.add.text(300,220,'Cat Life: ', {font : '18px Arial', fill: '#ffffff'});
		this.finalScore = game.add.text(300,180,'Green Balloons: ', {font : '18px Arial', fill: '#ffffff'})
