var level1State = {
	create: function(){
		...
		this.levelLabel = game.add.text(30,10,'Hit Green Balloons! Level 1', {font : '18px Arial', fill: '#000'});
		this.scoreLabel = game.add.text(30,30,'Green Balloons Exploded: 0', {font : '18px Arial', fill: '#000'});
		this.predictedTPLabel = game.add.text(30,50,'Predicted Throughput: ' + (Math.log2(6)/4).toFixed(2), {font : '18px Arial', fill: '#000'});
	...
	killBalloon: function(sprite, pointer){
		...
		if(sprite.key == 'balloongreen') {
			...
			//this.actualTPLabel.text = "Game Play Throughput:" + tp;
		}
		// else{
		// 	this.lifes--;
		// 	this.heartsGroup.children[this.lifes].kill();
		...
		if(this.counterGreenBalloons == this.pointsLimit){
			...
			game.time.events.stop();
			this.win = true;
			this.messageGameOver = "Good Job Passing Level 1!";
			this.freeCat();
			//this.gameOver();
		}
